        ...
        <Swiper slidesPerView={1} pagination={{clickable: true}} >
            {listing.imageUrls.map((url,index) => (
                <SwiperSlide key={index} >
                    <div className="swiperSlideDiv" style={{background: `url(${listing.imageUrls[index]}) no-repeat center center/cover`, position: 'relative' }}></div>
                </SwiperSlide>
            ))}
        </Swiper>
       ...
       <div className="listingDetails">
           ...
           <p className="listingType">
            ...
            {auth.currentUser?.uid !== listing.userRef && (
                <Link className='primaryButton' to={`/contact/${listing.userRef}?listingName=${listing.name}`}>
                    Contact Landlord
                </Link>
            )}
